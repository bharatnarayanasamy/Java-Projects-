...
public class CardManager implements MouseListener {
	...
	private jrJava.memoryGame.DrawingPanel panel;
	...
	public void setDrawingPanel(jrJava.memoryGame.DrawingPanel panel){
		this.panel = panel;
			...
			if(cards[i].isSelected(e)){
				...
				if(nonSelected){
					...
					for(int j=0;j<cards.length;j++){
						if(cards[j].hasSameImage(firstSelected) && cards[j]!=firstSelected){
							panel.setTitle(j+1);
							break;
						}
					}
					...
					/*for(int j=0;j<cards.length;j++){
						if(cards[j].hasSameImage(firstSelected) && cards[j]!=firstSelected) cards[j].swap(secondSelected);  break;
					}*/
					panel.setTitle(-1);
					secondSelected.show();
